...
import { ScoutStateSchema, ScoutPreferencesSchema, RepoScoreSchema } from './schemas.js';
...
describe('ScoutStateSchema', () => {
  ...
  it('applies preference defaults', () => {
    ...
    expect(state.preferences.aiPolicyBlocklist).toEqual(['matplotlib/matplotlib']);
  });
  ...
  it('accepts merged and closed PRs', () => {
    const state = ScoutStateSchema.parse({
      version: 1,
      mergedPRs: [{ url: 'https://github.com/o/r/pull/1', title: 'Fix bug', mergedAt: '2025-01-01T00:00:00Z' }],
      closedPRs: [{ url: 'https://github.com/o/r/pull/2', title: 'Rejected', closedAt: '2025-01-01T00:00:00Z' }],
    });
...
describe('ScoutPreferencesSchema', () => {
  ...
  it('validates scope enum values', () => {
    const prefs = ScoutPreferencesSchema.parse({ scope: ['beginner', 'intermediate'] });
  ...
  it('validates project category enum values', () => {
    const prefs = ScoutPreferencesSchema.parse({ projectCategories: ['devtools', 'education'] });
