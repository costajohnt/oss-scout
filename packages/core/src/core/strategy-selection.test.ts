...
vi.mock('./github.js', () => ({
  getOctokit: () => ({}),
  checkRateLimit: vi.fn().mockResolvedValue({ remaining: 100, limit: 100, resetAt: new Date().toISOString() }),
}));
...
vi.mock('./search-phases.js', () => ({
  ...
  filterVetAndScore: vi.fn().mockResolvedValue({ candidates: [], allVetFailed: false, rateLimitHit: false }),
  searchInRepos: vi.fn().mockResolvedValue({ candidates: [], allBatchesFailed: false, rateLimitHit: false }),
...
function makeFakeCandidate(repo: string, priority: string) {
  return {
    ...
    vettingResult: {
      ...
      checks: { noExistingPR: true, notClaimed: true, projectActive: true, clearRequirements: true, contributionGuidelinesFound: true },
      notes: [],
    },
    projectHealth: {
      repo,
...
const { searchInRepos, searchWithChunkedLabels } = await import('./search-phases.js');
...
describe('Strategy Selection', () => {
  ...
  it('returns strategiesUsed in the result', async () => {
    const stateReader = { ...baseStateReader, getReposWithMergedPRs: () => ['owner/repo'] };
  ...
  it('only runs orgs strategy when specified', async () => {
    ...
    (searchWithChunkedLabels as ReturnType<typeof vi.fn>).mockResolvedValue([
      { html_url: 'https://github.com/myorg/repo/issues/1', repository_url: 'https://api.github.com/repos/myorg/repo', updated_at: new Date().toISOString() },
    ]);
    mockVetIssuesParallel.mockResolvedValue({
  ...
  it('runs all strategies when "all" is specified', async () => {
    ...
    const result = await discovery.searchIssues({ strategies: ['all'], maxResults: 100 });
    expect(result.strategiesUsed).toContain('merged');
  });
  ...
  it('uses defaultStrategy from preferences when no options.strategies', async () => {
    const prefs = { ...basePreferences, defaultStrategy: ['merged'] as SearchStrategy[] };
    const stateReader = {
      ...baseStateReader,
      getReposWithMergedPRs: () => ['owner/repo'],
      getStarredRepos: () => ['owner/starred'],
  ...
  it('can combine multiple strategies', async () => {
    ...
    const discovery = new IssueDiscovery('token', basePreferences, stateReader);
    const result = await discovery.searchIssues({ strategies: ['merged', 'starred'], maxResults: 100 });
    expect(result.strategiesUsed).toContain('merged');
  ...
  it('throws ValidationError when enabled phase has no data', async () => {
    const discovery = new IssueDiscovery('token', basePreferences, baseStateReader);
    await expect(discovery.searchIssues({ strategies: ['merged'] })).rejects.toThrow(
