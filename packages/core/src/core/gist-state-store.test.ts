...
describe('GistStateStore', () => {
  ...
  describe('bootstrap', () => {
    ...
    it('paginates gist search up to 5 pages', async () => {
      const octokit = makeOctokit({
        list: vi.fn()
          .mockResolvedValueOnce({ data: [{ id: 'g1', description: 'other1' }] })
          .mockResolvedValueOnce({ data: [{ id: 'g2', description: 'other2' }] })
          .mockResolvedValueOnce({ data: [{ id: 'g3', description: 'other3' }] })
          .mockResolvedValueOnce({ data: [{ id: 'g4', description: 'other4' }] })
          .mockResolvedValueOnce({ data: [{ id: 'g5', description: 'other5' }] }),
    ...
    it('writes gist ID to local cache on bootstrap', async () => {
      ...
      const cached = fs.readFileSync(path.join(tempDir, 'gist-id'), 'utf-8').trim();
      expect(cached).toBe('new-gist-id');
    });

    it('writes state cache on bootstrap', async () => {
      const state = makeState({ preferences: { githubUsername: 'cached-user' } });
      const octokit = makeOctokit({
        list: vi.fn().mockResolvedValue({
    ...
    describe('degraded mode', () => {
      it('falls back to local cache on API failure', async () => {
        const cachedState = makeState({ preferences: { githubUsername: 'cached' } });
        fs.writeFileSync(
...
describe('mergeStates', () => {
  ...
  it('unions mergedPRs by URL', () => {
    const local = makeState({
      mergedPRs: [
        { url: 'https://github.com/a/b/pull/1', title: 'PR1', mergedAt: '2026-01-01T00:00:00Z' },
        { url: 'https://github.com/a/b/pull/2', title: 'PR2', mergedAt: '2026-01-02T00:00:00Z' },
      ],
    });
    const remote = makeState({
      mergedPRs: [
        { url: 'https://github.com/a/b/pull/2', title: 'PR2-updated', mergedAt: '2026-01-02T00:00:00Z' },
        { url: 'https://github.com/a/b/pull/3', title: 'PR3', mergedAt: '2026-01-03T00:00:00Z' },
      ],
    });
  ...
  it('unions closedPRs by URL', () => {
    const local = makeState({
      closedPRs: [
        { url: 'https://github.com/a/b/pull/1', title: 'PR1', closedAt: '2026-01-01T00:00:00Z' },
      ],
    });
    const remote = makeState({
      closedPRs: [
        { url: 'https://github.com/a/b/pull/2', title: 'PR2', closedAt: '2026-01-02T00:00:00Z' },
      ],
    });
