...
const guidelinesCache = new Map<
  string,
  { guidelines: ContributionGuidelines | undefined; fetchedAt: number }
>();
...
function pruneCache(): void {
  ...
  if (guidelinesCache.size > CACHE_MAX_SIZE) {
    const entries = Array.from(guidelinesCache.entries()).sort(
  ...
  try {
    return await cachedTimeBased(
      ...
      async () => {
        ...
        const lastCommitAt =
          lastCommit?.commit?.author?.date || repoData.pushed_at;
