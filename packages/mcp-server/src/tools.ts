...
export function registerTools(server: McpServer, scout: OssScout): void {
  ...
  const ARRAY_KEYS = new Set([
    'languages', 'labels', 'preferredOrgs', 'projectCategories',
    'excludeRepos', 'aiPolicyBlocklist', 'scope', 'defaultStrategy',
  ]);
  const NUMBER_KEYS = new Set(['minStars', 'maxIssueAgeDays', 'minRepoScoreThreshold']);
  ...
  server.tool(
    ...
    'Update an oss-scout configuration preference',
    {
      ...
      value: z.string().describe('New value — comma-separated for arrays, plain string for scalars'),
    },
    async ({ key, value }) => {
      if (!VALID_KEYS.includes(key)) {
        return {
          content: [{ type: 'text', text: `Unknown config key: "${key}". Valid keys: ${VALID_KEYS.join(', ')}` }],
          isError: true,
        };
      }
      ...
      if (ARRAY_KEYS.has(key)) {
        ...
        try {
          ...
          if (!Array.isArray(parsed)) {
            parsed = value.split(',').map((s: string) => s.trim()).filter(Boolean);
          }
        } catch {
          parsed = value.split(',').map((s: string) => s.trim()).filter(Boolean);
        }
        ...
        if (isNaN(num)) {
          return {
            content: [{ type: 'text', text: `Invalid number for "${key}": "${value}"` }],
            isError: true,
          };
        }
        parsed = num;
        ...
        if (lower === 'true' || lower === 'yes') parsed = true;
          ...
          return {
            content: [{ type: 'text', text: `Invalid boolean for "${key}": "${value}". Use true/false or yes/no.` }],
            isError: true,
          };
        }
      } else {
